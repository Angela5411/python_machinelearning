...
#0:survives 1:non survived
tmpCount = sum(y_train == 1)
Nhealthy=3*tmpCount#519

healthy=[]
deadX=[]
#seperates healthy and dead companies
...
#keeps randomly Nhealthy healthy companies
healthyX=random.sample(healthy, Nhealthy)
#time to reassemble X_train
a=healthyX+deadX
b=list(np.zeros(len(healthyX),dtype=int))+list(np.ones(len(deadX),dtype=int))
...
for classIdx in range(0, len(np.unique(y_train))):
    tmpCount = sum(y_train == classIdx)
    tmpPercentage = tmpCount/len(y_train)
    print(' .. class', str(classIdx), 'has', str(tmpCount), 'instances', '(', '{:.2f}'.format(tmpPercentage), '%)')
...
try:
    ...
    writer = pd.ExcelWriter('./OutputData/Results2.xlsx', engine='xlsxwriter')
